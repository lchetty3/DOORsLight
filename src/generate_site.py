# DOORS CSV → Static HTML generator (v2, no ModulePath) — Fixed assets & links
#
# 🔧 What changed (bugfixes):
# - All pages now compute a proper relative prefix for assets and internal links.
#   • Root pages (index.html) use prefix="".
#   • Subpages (levels/*, requirements/*, edit/*) use prefix="../".
# - Requirement→requirement links now resolve correctly from any page depth.
# - Stylesheet and JS load on every page (no more 404 for style.css on subpages).
#
# Usage:
#   pip install pyyaml
...
def discover_module_files(exports_root: Path):
    ...
    for p in exports_root.rglob("*.csv"):
        if p.name.lower() == "requirements.csv":
            req_files.append(p)
        elif p.name.lower() == "tests.csv":
            test_files.append(p)
...
def requirement_url(eid: str) -> str:
    return f"requirements/{eid.replace('/', '_')}.html"


def module_edit_url(mod: str) -> str:
    return f"edit/edit-{mod}.html"


def level_url(level: str) -> str:
    return f"levels/{level.lower()}.html"


def prefix_by_depth(depth: int) -> str:
...
def render_index(proj: Project, out_root: Path) -> None:
    ...
    for lvl in ("System", "Subsystem", "Component"):
        cards.append(
            f"<a class='card' href='{level_url(lvl)}'><h3>{escape(lvl)} view</h3><p>Browse {escape(lvl.lower())} requirements and rollups.</p></a>"
        )
    mods = "".join(
        f"<li><strong>{escape(m.abbrev)}</strong> — {escape(m.name)} ({escape(m.level)}) · "
        f"<a href='{module_edit_url(m.abbrev)}'>edit links</a></li>" for m in proj.modules.values()
    )
    body = f"""
...
def group_requirements_by_level(proj: Project) -> Dict[str, List[Requirement]]:
    level_map: Dict[str, List[Requirement]] = {"System": [], "Subsystem": [], "Component": []}
...
def render_level_pages(proj: Project, out_root: Path) -> None:
    ...
    for lvl, reqs in grouped.items():
        ...
        for r in reqs:
            ...
            incoming_links = " ".join(
                f"<a href='{p}{requirement_url(e)}'>{escape(e)}</a>" for e in r.incoming if e in proj.requirements
            )
            outgoing_reqs = [e for e in r.outgoing if e in proj.requirements]
            outgoing_tests = [e for e in r.outgoing if is_test_id(e)]
            outgoing_html = "".join([
                (f"<div><strong>Req:</strong> " + ", ".join(f"<a href='{p}{requirement_url(e)}'>{escape(e)}</a>" for e in outgoing_reqs) + "</div>") if outgoing_reqs else "",
                (f"<div><strong>Tests:</strong> " + ", ".join(escape(e) for e in outgoing_tests) + "</div>") if outgoing_tests else "",
            ])
            rows.append(
                f"<tr>"
                f"<td><a href='{p}{requirement_url(r.external_id)}'>{escape(r.external_id)}</a></td>"
                f"<td>{escape(r.heading)}</td>"
                f"<td>{escape(truncate(r.text, 180))}</td>"
                f"<td>{incoming_links}</td>"
                f"<td>{outgoing_html}</td>"
                f"<td>{badge(label)}</td>"
                f"</tr>"
            )
        body = f"""
...
def render_requirement_pages(proj: Project, out_root: Path) -> None:
    ...
    for r in proj.requirements.values():
        ...
        for eid in r.incoming:
            ...
            if rr:
                inc_rows.append(
                    f"<tr><td><a href='{p}{requirement_url(rr.external_id)}'>{escape(rr.external_id)}</a></td>"
                    f"<td>{escape(rr.heading)}</td>"
                    f"<td>{escape(truncate(rr.text, 200))}</td></tr>"
                )
            else:
...
def render_edit_pages(proj: Project, out_root: Path) -> None:
    ...
    for mod, reqs in by_mod.items():
        ...
        for r in reqs:
            rows.append(
                f"<tr>"
                f"<td>{escape(r.external_id)}</td>"
                f"<td>{escape(r.heading)}</td>"
                f"<td class='wrap'>{escape(r.text)}</td>"
                f"<td class='code'>{escape(';'.join(r.incoming))}</td>"
                f"<td class='code' contenteditable='true' data-initial='{escape(';'.join(r.outgoing))}'>{escape(';'.join(r.outgoing))}</td>"
                f"</tr>"
            )
        body = f"""
        ...
        <div class='table-wrap'>
          <table id='reqTable' class='table'>
            <thead><tr><th>ExternalID</th><th>Heading</th><th>ObjectText</th><th>IncomingLinks</th><th>OutgoingLinks (editable)</th></tr></thead>
            <tbody>{''.join(rows)}</tbody>
          </table>
        </div>
...
@media print{
  ...
  .site-header,.site-footer,.toolbar,.nav{display:none !important}
